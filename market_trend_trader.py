...
def is_buy_condition(data):
    ...
    return 30 < recent_rsi <= 60  # RSI가 30과 60 사이일 때 매수
...
def is_sell_condition(current_price, avg_buy_price, trend):
    fee = 1.0005  # 수수료 0.05%
    if trend == "bull":
        if current_price >= avg_buy_price * fee * 1.005:  # 0.5% 이상 상승 시 수익 실현
            return "profit"
        elif current_price <= avg_buy_price * fee * 0.995:  # 0.5% 이하 하락 시 손실
            return "loss"
    elif trend == "bear":
        if current_price >= avg_buy_price * fee * 1.002:  # 0.2% 이상 상승 시 수익 실현
            return "profit"
        elif current_price <= avg_buy_price * fee * 0.99:  # 1% 이하 하락 시 손실
            return "loss"
    elif trend == "sideways":
        if current_price >= avg_buy_price * fee * 1.003:  # 0.3% 이상 상승 시 수익 실현
            return "profit"
        elif current_price <= avg_buy_price * fee * 0.997:  # 0.3% 이하 하락 시 손실
            return "loss"
